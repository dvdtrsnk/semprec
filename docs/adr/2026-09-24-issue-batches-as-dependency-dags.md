...
- `implement-issue`'s Relay workflow `max-concurrent` has moved from 2 to 3
  (`.relay/workflows/implement-issue.md`, shipped in a separate maintainer
  pull request since `.relay/**` is a protected path) so a batch that
  decomposes into three independent issues can actually run all three at
  once, instead of the DAG shape existing on paper while the runner still
  gated it to two.
