...
describe("MCP server database seed and proposal/confirm integration (issue #123)", () => {
  ...
  it("seeds the mcpServers database with a locked schema and the expected properties", async () => {
    const { rows } = await pool.query<{ schema_locked: boolean; system: boolean }>(
      "SELECT schema_locked, system FROM databases WHERE id = $1",
      [mcpServersId],
    );
    expect(rows[0]).toEqual({ schema_locked: true, system: true });

    const { rows: props } = await pool.query<{ key: string; owner: string }>(
      "SELECT key, owner FROM properties WHERE database_id = $1 ORDER BY key",
      [mcpServersId],
    );
  ...
  it("accepts a proposal with a valid stdio connectionConfig and no credential", async () => {
    const proposal = await createMcpProposal({
      name: "Local tool",
      connectionConfig: { transport: "stdio", command: "my-mcp-server" },
    });
  ...
  it("rejects a proposal whose connectionConfig has an unknown transport", async () => {
    await expect(createMcpProposal({ name: "Bad", connectionConfig: { transport: "carrier-pigeon" } })).rejects.toThrow(
      /Invalid MCP server connectionConfig/,
    );
  });

  it("rejects a proposal whose connectionConfig is missing required stdio fields", async () => {
    await expect(createMcpProposal({ name: "Bad", connectionConfig: { transport: "stdio" } })).rejects.toThrow(
      /Invalid MCP server connectionConfig/,
    );
  });

  it.each(MCP_CREDENTIAL_FIELD_NAMES)("rejects a proposal carrying a credential-shaped field '%s'", async (field) => {
    await expect(createMcpProposal({ name: "Sneaky", [field]: "shh" })).rejects.toThrow(
      /cannot carry credential field/,
    );
  });
  ...
  it("rejects a stdio connectionConfig whose env map smuggles a credential-shaped key", async () => {
    await expect(
      createMcpProposal({
        name: "Sneaky env",
        connectionConfig: { transport: "stdio", command: "my-mcp-server", env: { apiKey: "sk-shh" } },
      }),
  ...
  it("rejects a stdio connectionConfig env key that only matches the denylist once normalized (SCREAMING_SNAKE_CASE)", async () => {
    await expect(
      createMcpProposal({
        name: "Sneaky env",
        connectionConfig: { transport: "stdio", command: "my-mcp-server", env: { API_KEY: "sk-shh" } },
      }),
    ).rejects.toThrow(/cannot carry credential field/);
  });
  ...
  it("rejects an sse connectionConfig whose url query string smuggles a credential-shaped param", async () => {
    await expect(
      createMcpProposal({
        name: "Sneaky url",
        connectionConfig: { transport: "sse", url: "https://mcp.example.com/sse?token=sk-shh" },
      }),
    ).rejects.toThrow(/cannot carry credential field/);
  });
  ...
  it("rejects an http connectionConfig whose url query param only matches the denylist once normalized", async () => {
    await expect(
      createMcpProposal({
        name: "Sneaky url",
        connectionConfig: { transport: "http", url: "https://mcp.example.com/mcp?API_KEY=sk-shh" },
      }),
    ).rejects.toThrow(/cannot carry credential field/);
  });
  ...
  it("confirm with a credential atomically stores the item and the encrypted credential", async () => {
    const proposal = await createMcpProposal({
      name: "Remote tool",
      connectionConfig: { transport: "sse", url: "https://mcp.example.com/sse" },
    });
    ...
    expect(server!.properties).toEqual({
      name: "Remote tool",
      connectionConfig: { transport: "sse", url: "https://mcp.example.com/sse" },
    });
    ...
    const decrypted = await withTransaction(pool, (client) =>
      getDecryptedCredential(client, { itemId: serverId, actorType: "mcp_connection_manager", purpose: "test" }),
    );
