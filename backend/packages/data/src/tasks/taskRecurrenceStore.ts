...
import type { TaskRecurrenceRow } from "../types.js";
...
function mapRow(row: { item_id: string; mode: string; rule: unknown; active: boolean }): TaskRecurrenceRow {
  const mode = assertKnownValue(TASK_RECURRENCE_MODES, row.mode, "task recurrence mode");
  return {
    itemId: row.item_id,
    mode,
    rule: parseTaskRecurrenceRule(mode, row.rule) as unknown as Record<string, unknown>,
    active: row.active,
  };
}
...
export async function createTaskRecurrence(client: Queryable, input: CreateTaskRecurrenceInput): Promise<TaskRecurrenceRow> {
...
export async function getTaskRecurrence(client: Queryable, itemId: string): Promise<TaskRecurrenceRow | null> {
