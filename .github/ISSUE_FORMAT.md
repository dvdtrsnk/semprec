...
| `agent:blocked` | a Relay workflow's `on-blocked` chain (`implement-issue`, `merge-pull-request`, `fix-review-findings`, `triage-review-followups`), or a human | Park label: the run stopped deliberately on something only a human can decide. Excluded from every workflow's trigger; on a pull request, `recover-blocked-issue` may pick it up automatically on the same branch. On a harvest issue, `triage-review-followups` sets it to park the issue's triage. A human resolves the issue and removes the label to let Relay pick it up again |
