- **critical** — an ADR whose body misstates the architecture it claims to document, or a
  ...
  not call for, or one that weakens or removes a required check; a `write` permission
  scope or a secret reachable by pull-request-controlled code in a workflow triggered
  by `pull_request_target`, or a move from an explicit `permissions:` block to the
  default token permissions (see `tasks/ci-workflows.md`).
- **high** — editing an existing ADR's Context/Decision/Consequences to reflect a new
  ...
  needs, a secret reachable by pull-request-controlled code (both outside the
  `pull_request_target` cases above), an event expression
